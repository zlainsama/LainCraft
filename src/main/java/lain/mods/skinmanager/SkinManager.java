...
public class SkinManager implements IPacketHandler, IConnectionHandler
{
    ...
    public void onPacketData(INetworkManager paramINetworkManager, Packet250CustomPayload paramPacket250CustomPayload, Player paramPlayer)
    {
        ...
        {
            if ("SkinServer".equals(var1[0]) && config_forceSkins)
                paramINetworkManager.addToSendQueue(new Packet250CustomPayload("LC|SM", ("SkinServer\u0000" + config_SkinServer).getBytes()));
            else if ("CloakServer".equals(var1[0]) && config_forceSkins)
                paramINetworkManager.addToSendQueue(new Packet250CustomPayload("LC|SM", ("CloakServer\u0000" + config_CloakServer).getBytes()));
        }
        else
        {
            if ("SkinServer".equals(var1[0]))
            {
                config_SkinServer = var1[1];
                System.out.println("LainCraft: SkinServer has been set to [" + config_SkinServer + "]");
            }
            else if ("CloakServer".equals(var1[0]))
            {
                config_CloakServer = var1[1];
                System.out.println("LainCraft: CloakServer has been set to [" + config_CloakServer + "]");
            }
        }
    }
    ...
    public void onUpdateSkins(ClientPlayerUpdateSkinsEvent event)
    {
        ...
        event.skinUrl = String.format(config_SkinServer, username);
        event.cloakUrl = String.format(config_CloakServer, username);
    }
